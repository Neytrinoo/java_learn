...
//        System.out.println(b2.width); // 10.0
        ...
        // КОНЕЦ 7 ГЛАВЫ. НАЧАЛО 8. НАСЛЕДОВАНИЕ
        System.out.println("\nНаследование");
        BoxWeight mybw1 = new BoxWeight(10, 20, 15, 34.3);
        System.out.println(mybw1.return_volume());
        Box mycopubw = new Box(10, 20, 300);
        mycopubw = mybw1; // Переменной суперкласса может быть присвоино значение переменной подкласса. При этом доступ к членам подкласса, неопределенных в суперклассе, эта переменная иметь не будет
        // System.out.println(mybw1.width); - член width также недоступен у подкласса, т.к. в суперклассе он приватный

        Shipment shipment1 = new Shipment(10, 20, 15, 10, 3.41);
        System.out.println(shipment1.return_volume());
    }
...
//        height = h;
//        depth = d;
        super(w, h, d); // вызов конструктора суперкласса. Аналогично self.__super__() в Python'е. Также super() ссылается на конструктор ближайшего по иерархии суперкласса. В данном случае BoxWeight
        weight = m;
    }

    BoxWeight(BoxWeight ob) {
        super(ob); // Несмотря на то, что поля width, height и depth приватны в суперклассе Box, с помощью super можно инициализировать подкласс с использованием конструктора суперкласса
        weight = ob.weight;
    }
...
class Shipment extends BoxWeight { // Наследуем подкласс от подкласса суперкласса. Для добавления поля стоимости доставки
    double cost;

    Shipment(double w, double h, double d, double m, double c) {
        super(w, h, d, m); // вызов конструктора суперкласса. Аналогично self.__super__() в Python'е
        cost = c;
    }
